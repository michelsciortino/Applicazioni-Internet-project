...
export class NotificationService {
    ...
    constructor(private parentSvc: ParentService, private authService: AuthService) {
        this.connectedSbj = new BehaviorSubject(false);
        this.unreadSbj = new BehaviorSubject(0);
        this.notifications = [];
        this.notifier = new EventEmitter();
        this.loggedStatusSub = this.authService.observeLoggedStatus()
            .subscribe(
                (status) => {
    ...
    public notify(notification?: Notification) {
        !this.stompClient || this.stompClient.send('/app/notify', null, JSON.stringify({
            performerUsername: this.authService.getCurrentUser().mail,
    ...
    public subscribeToTodayRaces() {
        if (this.connectedSbj.value) {
            this.parentSvc.getTodayRaces()
                .then((races: Race[]) => races.forEach(race => this.subscribeToRace(race.line.name, race.date, race.direction)))
                .catch(error => console.log(error))
        }
    }

    private subscribeToRace(lineName: string, date: Date, direction: DirectionType) {
        this.stompClient.subscribe(`/topic/notifications/${lineName}/${date.toISOString()}/${direction}`,
            (notification: any) => {
                this.notifications.push(notification);
